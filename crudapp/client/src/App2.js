...
const handleLoadMore = () => {
  ...
  return (
        ...
        <div className="table-title">
          <div className="row">
            ...
            <div className="col-sm-6">
              ...
              <div onClick={handleDeleteSelected} className="btn btn-danger">Delete</div>						
            </div>
          </div>
        </div>
        <table className="table table-striped table-hover">
          ...
          {currentItems? currentItems.map((item) => (
            <tr key={item.id}>
              ...
              <td>{item.email}</td>
              <td>
                <div onClick={() => handleOpenModal(item.id)} className="edit" ><FiEdit></FiEdit></div>
        <a onClick={() => handleDelete(item.id)} className="delete"><FiTrash2></FiTrash2></a>
              </td>
            </tr>
    ...
    <tr key={values._id}>
      <td>
        <span className="custom-checkbox">
        <input type="checkbox" checked={selectedRows.includes(id)} onChange={e => handleRowSelect(e, id)} />
